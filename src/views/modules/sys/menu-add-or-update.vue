  ...
  export default {
    ...
    methods: {
      init (id, isAdd) {
        if (isAdd === 'iaAdd') {
          this.isAdd = true
          this.dataForm.moduleCode = id
        } else {
          this.dataForm.id = id
        }
        this.$http({
          url: this.$http.adornUrl(this.$API.SYS_INFO.MENU.MENU_SELECT),
          method: 'get',
          ...
          if (this.isAdd) {
            ...
            // 修改
            this.$http({
              url: this.$http.adornUrl(`${this.$API.SYS_INFO.MENU.MENU_INFO}/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then((data) => {
              console.log(data)
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`${this.isAdd ? this.$API.SYS_INFO.MENU.MENU_SAVE : this.$API.SYS_INFO.MENU.MENU_UPDATE}`),
              method: 'post',

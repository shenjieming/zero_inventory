  ...
  export default {
    ...
    computed: {
      ...
      menuActiveName: {
        ...
        set (val) { this.$store.commit('permission/updateMenuActiveName', val) }
      },
      mainTabs: {
    ...
    methods: {
        ...
        if (this.mainTabs.length >= 1) {
          ...
          if (tabName === this.mainTabsActiveName) {
            this.$router.push({ name: this.mainTabs[this.mainTabs.length - 1].name }, () => {
              this.$store.commit('permission/updateMainTabsActiveName', this.$route.name)
            })
          }
        } else {
          this.$store.commit('permission/updateMainTabsActiveName', '')
      ...
      tabsCloseOtherHandle () {
        const arr = this.mainTabs.filter(item => item.name === this.mainTabsActiveName)
        this.$store.commit('permission/updateMainTabs', arr)
      },
      // tabs, 关闭全部
      tabsCloseAllHandle () {
